...
public class Ammo {
	...
	//
	// AMMO_TABLE  : ammo name -> fully-constructed Ammo object
	// CALIBER_POOL: caliber string -> list of ammo names belonging to that caliber
	//
	// Both are populated once by the static initialiser below via register().
	// Nothing else in the codebase needs to change: Gun still calls
	// Ammo.randomForCaliber(caliber) and Ammo.get(name) as before.
	...
	static {
		...
		// ── .45 ACP ──────────────────────────────────────────────────────────
		// "RIP" already registered as 9x19mm above; .45 ACP RIP is distinct —
		// different stats, so we use a disambiguated key.
		...
		register("BS GS",              "5.45x39mm",   1,  44, 55); // kept from original array
	...
	/*
	 * Builds one Ammo object from the provided stats, stores it in AMMO_TABLE
	 * under its name, and adds the name to the correct CALIBER_POOL bucket.
	 * "BS GS" appears in both 5.45x39mm and 7.62x54mmR with different stats;
	 * the second call overwrites the first in AMMO_TABLE, so Gun must request
	 * the caliber pool rather than look up by name when those pools overlap.
	 */
	private static void register(String name, String caliber,
	                              int projectiles, int fleshDmg, int pen) {
		Ammo a = new Ammo();
		a.name            = name;
		a.size            = caliber;
	...
	/**
	 * Returns a random Ammo from the given caliber's pool.
	 * This is the primary method Gun should call when loading a weapon.
	 *
	 * Example:  Ammo.randomForCaliber("9x19mm")
	 */
	...
	/**
	 * Returns the Ammo object for a specific named round, or an empty Ammo if
	 * not found. Useful when a boss or specific loadout must carry a fixed round.
	 *
	 * Example:  Ammo.get("M995")
	 */
	...
	// ── Accessors ────────────────────────────────────────────────────────────
	/*
	 * Setters are kept for compatibility with any code that still constructs
	 * Ammo objects manually, but prefer register() + get() / randomForCaliber().
	 */
	...
	public int  getFleshDamage()                     { return fleshDamage; }
	public void setFleshDamage(int fleshDamage)      { this.fleshDamage = fleshDamage; }

	public int  getPenetrationPower()                { return penetrationPower; }
	public void setPenetrationPower(int pen)         { this.penetrationPower = pen; }
}